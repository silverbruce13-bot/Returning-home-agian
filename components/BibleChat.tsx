...
const BibleChat: React.FC<BibleChatProps> = ({ passage }) => {
  ...
  useEffect(() => {
    ...
    // Fix: Updated model name to gemini-3-flash-preview for general text tasks
    const newChat = ai.chats.create({
        model: 'gemini-3-flash-preview',
        config: {
            systemInstruction: systemInstruction,
        },
        history: chatHistoryForAi as any, // Cast to any to handle library internal content type matching
    });
  ...
  const handleSendMessage = async () => {
    ...
    try {
      ...
      for await (const chunk of result) {
        // Fix: Properly accessing .text property on the GenerateContentResponse chunk
